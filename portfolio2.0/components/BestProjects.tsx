      ...
      <section className="w-full">
        ...
        {projectData.map((project,index) => (
          <div key={project.id} className="flex flex-col  mt-6 py-4 cursor-pointer">
            ...
            <span className="flex justify-between">{project.name}{selected === index ? <FiMinus className=" text-xs h-4 w-4 mt-4 dark:text-white"/> : <FiPlus className="text-xs h-4 w-4 mt-4 py-auto dark:text-white"/>}</span>
               { selected === index ? (
                <>
                <div className="w-2/3 text-sm font-cd-regular dark:text-white">{project.description}</div>
                <span className="flex">
